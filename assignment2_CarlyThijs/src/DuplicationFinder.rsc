...
public void main(loc projectLocation, loc srcLocation, int duplicationType){
	...
	tuple[map[int, list[loc]] dupGroups, map[loc,int] locPerFile] dups = findDuplications(projectLocation, duplicationType);
	generateReport(projectLocation, dups.dupGroups, dups.locPerFile);
